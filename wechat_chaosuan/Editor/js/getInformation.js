...
function getInformation(_nowanpage) {
    ...
    request.open('GET', './php/getarticle.php?page=' + _nowanpage);
    // 将文章列表添加到文档流里
       	...
       	if (request.readyState === 4) {
       		if (request.status === 200) {
                ...
                for (var i = 1, len = trs.length; i < len; i++) {
                    // “删除”添加到每一行的第五个<td>内
                   	var removeonetr = trs[i].getElementsByTagName('td')[4];
                    // “修改”添加到每一行的第六个<td>内
                    var updateonetr = trs[i].getElementsByTagName('td')[5];
                    // “预览”添加到每一行的第七个<td>内
                    var previewonetr = trs[i].getElementsByTagName('td')[6];
                    // 添加删除事件
                    ...
                    updateonetr.onclick = (function (updateonetr) {
                        return function () {
                            ...
                            // 拼接请求
                            request.open('GET', './php/getoneArticle.php?time=' + updatetime);
                            request.send();
                            request.onreadystatechange = function () {
                                if (request.readyState === 4) {
                                    if (request.status === 200) {
